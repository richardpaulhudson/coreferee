...
class CommonRulesTest(unittest.TestCase):
    ...
    def test_sent_starts(self):
        self.compare_sent_starts(
            "My name is Charles. I am here. The weather is good", [0, 5, 9]
        )
    ...
    def compare_sent_indexes(self, doc_text, expected_sent_indexes):
        def func(nlp):
            ...
            rules_analyzer.initialize(doc)
            self.assertEqual(
                expected_sent_indexes,
                [token._.coref_chains.temp_sent_index for token in doc],
                nlp.meta["name"],
            )
    ...
    def test_sent_index(self):
        self.compare_sent_indexes(
            "My name is Charles. I am here. The weather is good",
    ...
    def compare_quote_array(self, doc_text, index, expected_quote_array):
        def func(nlp):
            ...
            rules_analyzer.initialize(doc)
            self.assertEqual(
                expected_quote_array,
                doc[index]._.coref_chains.temp_quote_array,
                nlp.meta["name"],
            )
    ...
    def test_potential_noun_pair_proper_noun_referred_multiword(self):
        self.compare_potential_noun_pair(
            "This is Peter Smith. Peter Smith is here", 3, 6, True
        )

    def test_potential_noun_pair_proper_noun_referred_multiword_referring_end(self):
        self.compare_potential_noun_pair(
            "This is Peter Smith. Smith is here", 3, 5, True
        )

    def test_potential_noun_pair_proper_noun_referred_multiword_referring_beginning(
        self,
    ):
        self.compare_potential_noun_pair(
            "This is Peter Smith. Peter is here", 3, 5, False
        )

    def test_potential_noun_pair_referred_proper_noun_with_child(self):
        self.compare_potential_noun_pair(
            "I spoke to big Peter. Peter is here", 4, 6, True
        )

    def test_potential_noun_pair_referred_proper_noun_conjunction_first_member(self):
        self.compare_potential_noun_pair(
            "I spoke to Peter and Jane. Peter is here", 3, 7, True
        )

    def test_potential_noun_pair_referred_proper_noun_conjunction_second_member(self):
        self.compare_potential_noun_pair(
            "I spoke to Peter and Jane. Jane is here", 5, 7, True
        )

    def test_potential_noun_pair_referring_back_proper_noun_conjunction_first_member(
        self,
    ):
        self.compare_potential_noun_pair(
            "I spoke to Peter. Peter and Jane are here", 3, 5, True
        )

    def test_potential_noun_pair_referring_back_proper_noun_conjunction_second_member(
        self,
    ):
        self.compare_potential_noun_pair(
            "I spoke to Jane. Peter and Jane are here", 3, 7, True
        )
    ...
    def test_potential_noun_pair_definite_plural_common_noun_referred_2(self):
        self.compare_potential_noun_pair(
            "These are some men. The men are here", 3, 6, True
        )

    def test_potential_noun_pair_definite_plural_common_noun_referred_3(self):
        self.compare_potential_noun_pair(
            "These are the men. The men are here", 3, 6, True
        )
    ...
    def test_potential_noun_pair_numbers_do_not_match(self):
        self.compare_potential_noun_pair(
            "This is a man. The men are here.", 3, 6, False
        )
    ...
    def test_potential_noun_pair_single_letters(self):
        self.compare_potential_noun_pair("This is a ©. The © is here.", 3, 6, False)
            ...
            if consider_syntax:
                ...
                self.assertEqual(
                    ...
                    rules_analyzer.is_potential_anaphoric_pair(
                        referred_mention, doc[referring_index], False
                    ),
                    nlp.meta["name"],
                )
    ...
    def test_closer_within_structure_propn_conjunction_first(self):
        ...
        self.compare_potential_pair(
            "Richard and Peter arrived. Richard saw him.", 0, False, 7, 1
        )

    def test_closer_within_structure_propn_conjunction_second(self):

        self.compare_potential_pair(
            "Richard and Peter arrived. Peter saw him.", 2, False, 7, 1
        )

    def test_closer_within_structure_child(self):

        self.compare_potential_pair(
            "The dog arrived. The big dog saw him.", 1, False, 8, 2
        )
    ...
    def test_closer_within_structure_only_determiner_conjunction_first(self):
        ...
        self.compare_potential_pair(
            "The dog and the cat arrived. The dog saw him.", 1, False, 10, 1
        )

    def test_closer_within_structure_only_determiner_conjunction_second(self):

        self.compare_potential_pair(
            "The dog and the cat arrived. The cat saw him.", 4, False, 10, 1
        )
    ...
    def test_propn_subtree_beginning(self):
        ...
        self.assertEqual(
            [], [t.i for t in self.sm_rules_analyzer.get_propn_subtree(doc[0])]
        )
        self.assertEqual(
            [0, 1], [t.i for t in self.sm_rules_analyzer.get_propn_subtree(doc[1])]
        )
        self.assertEqual(
            [], [t.i for t in self.sm_rules_analyzer.get_propn_subtree(doc[3])]
        )
    ...
    def test_propn_subtree_middle(self):
        ...
        self.assertEqual(
            [3, 4], [t.i for t in self.sm_rules_analyzer.get_propn_subtree(doc[4])]
        )
        self.assertEqual(
            [], [t.i for t in self.sm_rules_analyzer.get_propn_subtree(doc[5])]
        )
    ...
    def test_propn_subtree_with_coordination(self):
        ...
        self.assertEqual(
            [], [t.i for t in self.sm_rules_analyzer.get_propn_subtree(doc[2])]
        )
        self.assertEqual(
            [], [t.i for t in self.sm_rules_analyzer.get_propn_subtree(doc[3])]
        )
    ...
    def compare_potentially_referring(
    ...
    def test_has_morph_without_value(self):
        def func(nlp):
            ...
            rules_analyzer.initialize(doc)
            self.assertEqual(
                True, rules_analyzer.has_morph(doc[0], "Number"), nlp.meta["name"]
            )
            self.assertEqual(
                False, rules_analyzer.has_morph(doc[0], "Other"), nlp.meta["name"]
            )
    ...
    def test_has_morph_with_value(self):
        def func(nlp):
            ...
            self.assertEqual(
                True,
                rules_analyzer.has_morph(doc[0], "Number", "Plur"),
                nlp.meta["name"],
            )
            self.assertEqual(
                False,
                rules_analyzer.has_morph(doc[0], "Number", "Other"),
                nlp.meta["name"],
            )
            self.assertEqual(
                False,
                rules_analyzer.has_morph(doc[0], "Other", "Other"),
                nlp.meta["name"],
            )
    ...
    def test_potentially_introducing_definite_noun_with_dependent_phrase(self):
        self.compare_potentially_introducing(
            "I saw the man whom we had discussed", 3, True, excluded_nlps="core_web_sm"
        )
    ...
    def test_potentially_introducing_threeway_conjunction_second_member_no_article(
        self,
    ):
        self.compare_potentially_introducing(
            "I spoke to some men, women and children", 6, True
        )

    def test_potentially_introducing_threeway_conjunction_third_member_no_article(self):
        self.compare_potentially_introducing(
            "I spoke to some men, women and children", 8, True
        )

    def test_potentially_introducing_twoway_conjunction_second_member_no_article_control(
        self,
    ):
        self.compare_potentially_introducing("I spoke to the men and women", 6, False)

    def test_potentially_introducing_threeway_conjunction_second_member_no_article_control(
        self,
    ):
        self.compare_potentially_introducing(
            "I spoke to the men, women and children", 6, False
        )

    def test_potentially_introducing_threeway_conjunction_third_member_no_article_control(
        self,
    ):
        self.compare_potentially_introducing(
            "I spoke to the men, women and children", 8, False
        )
    ...
    def test_potentially_referring_back_noun_definite_noun_with_dependent_phrase(self):
        self.compare_potentially_referring_back_noun(
            "I saw the man whom we had discussed", 3, False, excluded_nlps="core_web_sm"
        )

    def test_potentially_referring_back_noun_common_noun_conjunction_first_member(self):
        self.compare_potentially_referring_back_noun(
            "I spoke to the man and the woman", 4, True
        )

    def test_potentially_referring_back_noun_common_noun_conjunction_second_member(
        self,
    ):
        self.compare_potentially_referring_back_noun(
            "I spoke to the man and the woman", 7, True
        )

    def test_potentially_referring_back_twoway_conjunction_second_member_no_article(
        self,
    ):
        self.compare_potentially_referring_back_noun(
            "I spoke to the men and women", 6, True
        )

    def test_potentially_referring_back_threeway_conjunction_second_member_no_article(
        self,
    ):
        self.compare_potentially_referring_back_noun(
            "I spoke to the men, women and children", 6, True
        )

    def test_potentially_referring_back_threeway_conjunction_third_member_no_article(
        self,
    ):
        self.compare_potentially_referring_back_noun(
            "I spoke to the men, women and children", 8, True
        )

    def test_potentially_referring_back_twoway_conjunction_second_member_no_article_control(
        self,
    ):
        self.compare_potentially_referring_back_noun(
            "I spoke to some men and women", 6, False
        )

    def test_potentially_referring_back_threeway_conjunction_second_member_no_article_control(
        self,
    ):
        self.compare_potentially_referring_back_noun(
            "I spoke to some men, women and children", 6, False
        )

    def test_potentially_referring_back_threeway_conjunction_third_member_no_article_control(
        self,
    ):
        self.compare_potentially_referring_back_noun(
            "I spoke to some men, women and children", 8, False
        )
